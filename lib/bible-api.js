...
BibleAPI.createTranslation = function(token, bible_Id, bible_langcode, bible_version, bible_sourceBibleId, bible_status, callback) {

  var options = {
    url: BibleAPI.buildUrl('/api/bibles/' + bible_sourceBibleId + '/translations'),
    headers : {
      'Accept': 'application/json',
      'Authorization': 'Token token=' + token
    },
    json: {
        "translations" : [{
          ...
          "langCode": bible_langcode,
          "sourceBibleId": bible_sourceBibleId,
         "status": bible_status
        }]
      }
    };
...
BibleAPI.exportFile = function(token, bibleId, bookId, chapterId, transBibleId, callback) {
	...
	processedRes.forEach(function(item) {
		...
		filename = bibleId + '_' + bookId + '_' + chapterId + '_' + item.langCode + '.xlsx';
		var options = {
		    url: BibleAPI.buildUrl('/api/bibles/' + bibleId + '/books/' + bookId + '/chapters/' + chapterId + '/' + item.langCode + '/' + filename),
		    headers : {
			'Accept': 'application/json',
			'Authorization': 'Token token=' + token
		    }
